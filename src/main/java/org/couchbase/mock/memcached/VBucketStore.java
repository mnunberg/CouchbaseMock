...
public class VBucketStore {
    ...
    private final Map<Integer, List<VBucketCoordinates>> allCoords = new HashMap<Integer, List<VBucketCoordinates>>();
    ...
    private void logCoords(int vbid, VBucketCoordinates coords) {
        List<VBucketCoordinates> log = allCoords.get(vbid);
        if (log == null) {
            log = new ArrayList<VBucketCoordinates>();
            allCoords.put(vbid, log);
        }
        for (VBucketCoordinates cur : log) {
